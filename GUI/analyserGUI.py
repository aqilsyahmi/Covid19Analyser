...
def showGraph(month, year):
    # create a figure
    figure1 = plt.Figure(figsize=(18, 8), dpi=100, tight_layout=True)
...
cleanDataset = sd.severityRateDF.join(rd.mergeDataFrame[['Daily Discharged', 'Daily Recovery']])
cleanDataset = cleanDataset.join(vd.newdf)
cleanDataset
cleanDataset.to_csv("clean_dataset.csv", index=True)
